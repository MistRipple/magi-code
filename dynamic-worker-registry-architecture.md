...
- **展示身份**：`agentId = templateId`，用于 tab、lane、卡片、消息归属
- **执行规格版本**：由绑定关系和运行时策略派生，用于恢复和续跑

原因：

- 用户可以保留同一个角色身份，但切换绑定引擎
- 用户也可以调整 `profileOverrides`、停滞策略等执行参数
- 若恢复链只记录 `agentId`，则历史未完成执行会和新配置混淆

因此：

- UI / projection / 持久化主键继续使用稳定的 `agentId`
- Worker Session、Dispatch Resume、未完成任务恢复必须额外持久化 `bindingRevision` 或等价的 `executionSpecSnapshot`
- 任何长生命周期执行上下文都不能仅凭 `agentId` 反查"当前配置"来恢复
 ...
 * 长生命周期执行快照
 *
 * 用于 Worker Session、ResumeContext、未完成任务、恢复链。
 * 目标不是替代 Registry，而是保证"历史执行按当时规格恢复"。
 */
...
1. 用户视角下，同一个角色身份始终稳定
2. 恢复视角下，同一次执行必须绑定到当时的执行规格

因此强制采用：

- `agentId = templateId`：稳定展示身份
- `bindingRevision`：绑定关系版本号
- `AgentExecutionSnapshot`：写入 Worker Session / ResumeContext / 长任务恢复上下文

版本递增触发条件：

- `engineId` 变化
- `profileOverrides` 变化
- 会影响执行规格的 runtime 策略变化
...
### 8.4 Workspace Registry 广播协议

当前产品同时支持插件模式、Web 模式、以及多个工作区并行连接同一后端服务。

因此 Registry 不能只存在于某个前端页面的本地状态中，必须具备工作区级别的单一广播协议。

建议强制定义：

```ts
interface WorkspaceAgentRegistrySnapshot {
  workspaceId: string;
  registryVersion: number;
  engines: ModelEngine[];
  agents: AgentBinding[];
  readyAgents: AgentExecutionSpec[];
  updatedAt: number;
}
```
...
- `registryVersion` 由后端按工作区单调递增
- bootstrap 必须内联当前 `WorkspaceAgentRegistrySnapshot`
- 后端只广播一种更新事件：`workspaceRegistryUpdated`
- 插件端、Web 端、移动 Web 都只消费这一个快照事件，不允许各自拼装局部状态
- 前端收到新版本后必须原子替换本地 Registry 状态，不做多来源 merge
...
- 当 `readyAgents.length > 0` 时，ownership 优先路由到就绪角色集合中的成员
- 当某个 ownership 当前没有就绪角色覆盖时，该 ownership 统一由 `orchestrator` 接管，不得向用户暴露 dispatch 失败
...
- "配置未覆盖某个 ownership" 属于产品允许状态，不属于系统错误

#### Fallback 原则

- 默认不跨 ownership 自动降级
- 若该 ownership 无就绪角色覆盖，则直接回到 `orchestrator`
- 只有在"编排者显式指定了某个 agentId，但该 agentId 已失效"时，才返回系统内部错误并触发重新规划
...
- 先从 bootstrap 中拿到当前 `WorkspaceAgentRegistrySnapshot`
- 再从 timelineProjection 中拿到当前 session 的 `agentOrder` 和 `agentRenderEntries`
- 再从任务卡片 / 生命周期卡片 / Worker Session 恢复上下文中拿到 `AgentExecutionSnapshot`
- 若历史 session 中存在当前 Registry 已禁用的角色，则以卡片中的 `agentDisplaySnapshot` 继续渲染只读 lane

恢复链约束：

- 历史展示恢复优先依赖 `agentDisplaySnapshot`
- 历史执行恢复优先依赖 `AgentExecutionSnapshot`
- 不允许只凭 `agentId` 回查"当前 Registry"来恢复旧执行
- 同一 session 的 panel 顺序必须由 `thread + agentOrder` 唯一决定，不允许前端临时重排
...
- `thread` 始终存在
- `readyAgents.length > 0` 时，就绪角色自动追加到 Tabs 中
...
`tabOrder = ['thread'] + readyAgents(order 排序) + archivedSessionAgents(agentOrder 剩余项)`

其中：

- `readyAgents`：当前 Registry 中已启用且就绪的角色
- `archivedSessionAgents`：当前 session 中存在历史 lane、但当前已不再就绪的角色

交互约束：

- `readyAgents` tab 可交互、可接收实时输出
- `archivedSessionAgents` tab 只读展示，视觉上标明"历史"
- 若当前 session 中不存在该角色的历史 lane，则禁用角色不进入 Tabs

#### Tab 命名与标识

角色 Tab 统一使用角色模板的 `displayName` 作为标签文案：

- ✅ 正确：`主线` | `前端开发` | `后端开发` | `代码审查` | `测试工程师`
- ❌ 错误：`主线` | `Claude` | `Codex` | `Gemini`

Tab 不再以模型品牌命名，而是以角色身份命名。用户看到的是"谁在做什么"，而不是"用的什么模型"。

每个 Tab 的视觉标识（颜色、图标）来自 `RoleTemplate.defaultUI`，与模型品牌无关。

#### Tab 溢出处理

当就绪角色数量较多时（例如 8-15 个），Tab 栏必须支持水平滚动：

- `thread` tab 固定在最左侧，始终可见，不参与滚动
- 角色 tabs 区域独立横向滚动
- 支持左右滑动手势或滚轮水平滚动
- 当前选中的 tab 自动滚动到可见区域
- 不使用折叠/下拉菜单隐藏 tab，保证所有角色一目了然

布局结构：

```
┌──────────┬───────────────────────────────────────────────┐
│  主线 ⬤  │ ◀ 前端开发 │ 后端开发 │ 代码审查 │ 测试 │ ... ▶ │
│ （固定）  │        （可水平滚动区域）                      │
└──────────┴───────────────────────────────────────────────┘
```
...
- `effectiveAgentSet` 仅用于 UI 渲染与恢复，不代表都具备执行能力
- 是否可执行只看 `readyAgents`
- 是否需要展示只读历史 tab，只看 `agentOrder` 与历史 lane 是否存在
...
10. 当部分 ownership 未被角色覆盖时，系统自动由 orchestrator 接管，不向用户暴露路由失败
11. 插件端与 Web 端在同一后端服务下共享单一 Registry 快照与版本号，切换端侧后 Tabs / 路由 / 恢复结果一致
