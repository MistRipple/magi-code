 ...
 * 2) 跨主体 process_read/process_kill 必须被拒绝
 * 3) 同主体 process_read/process_kill 必须可执行
 */
...
async function main() {
  ...
  try {
    ...
    assert(orchLaunch && Number.isInteger(orchLaunch.terminal_id), 'orchestrator process_launch 失败');
    ...
    assert(workerLaunch && Number.isInteger(workerLaunch.terminal_id), 'worker process_launch 失败');
    launched.push({ terminalId: workerLaunch.terminal_id, workerId: 'claude' });
    assert(
      orchLaunch.terminal_id !== workerLaunch.terminal_id,
      '不同主体 process_launch 得到了相同 terminal_id',
    );
    ...
    assert(crossReadByOrchestrator.isError === true, 'orchestrator 跨主体 process_read 未被拒绝');
    ...
    assert(crossReadByWorker.isError === true, 'worker 跨主体 process_read 未被拒绝');
    ...
    assert(ownReadByOrchestrator.isError === false, 'orchestrator 自有 process_read 被错误拒绝');
    ...
    assert(ownReadByWorker.isError === false, 'worker 自有 process_read 被错误拒绝');
    ...
    assert(crossKillByOrchestrator.isError === true, 'orchestrator 跨主体 process_kill 未被拒绝');
    ...
    assert(ownKillByOrchestrator.isError === false, 'orchestrator 自有 process_kill 失败');
    ...
    assert(ownKillByWorker.isError === false, 'worker 自有 process_kill 失败');
