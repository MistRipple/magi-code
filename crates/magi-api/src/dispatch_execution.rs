...
use magi_core::{
    ...
    TaskStatus, ThreadId, UtcMillis, WorkerId, WorkspaceId,
};
...
// M12：任务图构建器与派发数据载体已迁移到 magi-conversation-runtime；保留内部 re-export
// 让 dispatch_execution / task_execution / 测试代码以原名继续调用。decompose_mission /
// parse_decomposition_plan 仍在 dispatch_execution.rs 内消费 TaskGraphPlan，所以
// TaskGraphPlan 需要 re-export；其它派生 Phase/Package/Action 类型只在 insert_task_graph
// 内部使用，未在本文件直接命名，因此不必 re-export。
pub(crate) use magi_conversation_runtime::task_graph_builder::{
    TASK_MAX_PHASES, TASK_MIN_PHASES, TaskGraphBuildResult, TaskGraphPlan, TaskGraphSubmission,
    build_task_policy, cleanup_task_tree, infer_dispatch_task_role, insert_task_graph,
    make_dispatch_task, task_phase_count_is_valid,
};
...
pub(crate) fn ensure_thread_for_role(
