当前工作区根目录是 `{{root_path}}`。当前运行平台是 `{{platform_name}}`，{{path_contract}}。文件工具优先传工作区相对路径；如需绝对路径，必须保持当前平台的原生格式，不得把 Windows 盘符路径改写成 Unix 路径，也不得反向改写。工具未显式传 cwd/root/path 的相对路径均按该工作区根目录理解。
