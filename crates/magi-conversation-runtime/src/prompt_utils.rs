...
7. 宽泛的“完整/全面分析项目”不等于逐文件盘点：先用目录、manifest、入口和搜索建立结构图，再抽样读取能证明架构、关键链路、配置、测试和风险的最小充分文件；不要为了覆盖率重复读取同类文件。
8. 独立的只读检索必须合并在同一轮并行调用；当结构、关键链路和风险证据已经覆盖时立即停止检索并综合结论。最终答复的广度来自结论覆盖面，不来自工具调用数量。";
