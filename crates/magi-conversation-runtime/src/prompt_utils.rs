...
2. 一旦用户明确要求 subagent / 子代理 / 多代理 / 派发代理，就必须通过 agent_spawn 创建真实代理，并提供最小充分的结构化 context_package；不得用主线直接读取、shell_exec 或口头总结冒充代理执行。\n\
...
5. root coordinator 保留主线推进职责：只把边界清晰、可并行、需要专项视角或独立复核的工作交给代理。代理运行中需要补充事实时使用 agent_send，不要等待下一次 Turn 或重启代理。";
