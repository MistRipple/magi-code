...
use crate::image_understanding::ImageUnderstandingProgress;
...
#[derive(Clone, Copy)]
pub(crate) struct ImageUnderstandingWritebackContext<'a> {
    pub(crate) event_bus: &'a InMemoryEventBus,
    pub(crate) session_store: &'a SessionStore,
    pub(crate) session_id: &'a SessionId,
    pub(crate) workspace_id: &'a Option<WorkspaceId>,
...
pub(crate) fn upsert_image_understanding_notice(
    context: ImageUnderstandingWritebackContext<'_>,
    progress: ImageUnderstandingProgress,
) {
    let (status, state, notice_type, image_count, title, content) = match progress {
        ImageUnderstandingProgress::Started { image_count } => (
            ...
            image_count,
            "正在理解图片",
            format!("正在理解 {image_count} 张图片"),
        ),
        ImageUnderstandingProgress::Completed { image_count } => (
            "completed",
            "completed",
            "success",
            image_count,
            "图片理解完成",
            format!("已完成 {image_count} 张图片的理解"),
        ),
        ImageUnderstandingProgress::Failed { image_count } => (
            ...
            image_count,
            "图片理解失败",
            format!("未能完成 {image_count} 张图片的理解"),
        ),
    ...
    item.metadata.insert(
        ...
        Value::String("image_understanding".to_string()),
    );
    item.metadata.insert(
    ...
    item.metadata.insert(
        "imageUnderstandingState".to_string(),
        Value::String(state.to_string()),
    );
    item.metadata
        .insert("imageCount".to_string(), Value::from(image_count as u64));
...
#[cfg(test)]
mod image_understanding_notice_tests {
    use super::*;
    use magi_session_store::ActiveExecutionTurn;

    #[test]
    fn image_understanding_notice_uses_one_canonical_item_across_states() {
        let event_bus = InMemoryEventBus::new(16);
        let session_store = SessionStore::new();
        let session_id = SessionId::new("session-image-understanding-notice");
        let workspace_id = Some(WorkspaceId::new("workspace-image-understanding-notice"));
        let thread_id = ThreadId::new("thread-image-understanding-notice");
        session_store
            .create_session(session_id.clone(), "image understanding notice")
            .expect("session should create");
        session_store
            .upsert_current_turn(
                session_id.clone(),
                ActiveExecutionTurn {
                    turn_id: "turn-image-understanding-notice".to_string(),
                    turn_seq: 1,
                    accepted_at: UtcMillis::now(),
                    completed_at: None,
                    status: "running".to_string(),
                    user_message: Some("看图".to_string()),
                    items: Vec::new(),
                },
            )
            .expect("current turn should store");
        let context = ImageUnderstandingWritebackContext {
            event_bus: &event_bus,
            session_store: &session_store,
            session_id: &session_id,
            workspace_id: &workspace_id,
            thread_id: &thread_id,
            item_id: "turn-item-image-understanding",
            persist_session_state: None,
            task: None,
            turn_visibility: None,
        };

        upsert_image_understanding_notice(
            context,
            ImageUnderstandingProgress::Started { image_count: 2 },
        );
        upsert_image_understanding_notice(
            context,
            ImageUnderstandingProgress::Completed { image_count: 2 },
        );
        ...
        let notice = &turn.items[0];
        assert_eq!(notice.status, "completed");
        assert_eq!(
            notice.metadata.get("noticeKind"),
            Some(&serde_json::json!("image_understanding"))
        );
        assert_eq!(
            notice.metadata.get("imageUnderstandingState"),
            Some(&serde_json::json!("completed"))
        );
        assert_eq!(
            notice.metadata.get("imageCount"),
            Some(&serde_json::json!(2))
        );
        let turn_item_events = event_bus
            .snapshot()
            .recent_events
            .into_iter()
            .filter(|event| event.event_type == "session.turn.item")
            .count();
        assert_eq!(turn_item_events, 2);
    }
}
