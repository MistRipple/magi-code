use crate::json_support::{field_string, parse_json_object, resolve_path};
use crate::{
    BuiltinToolAccessMode, BuiltinToolName, ToolExecutionContext, ToolExecutionInput,
    ToolExecutionOutput, ToolRegistry, WriteProtectionScope,
};
use magi_core::{ExecutionResultStatus, ToolCallId, UtcMillis};
...
impl ToolRegistry {
    ...
    fn build_write_scope(
        ...
        let working_directory = if input.tool_name == BuiltinToolName::ShellExec.as_str() {
            request
                .as_ref()
    ...
    fn build_write_conflict_rejection(
        &self,
        input: &ToolExecutionInput,
        access_mode: BuiltinToolAccessMode,
