...
pub enum BrowserAuthorityError {
    ...
    InvalidLeaseExpiry,
    #[error("browser profile is currently controlled by the user: {0}")]
    UserHasControl(BrowserProfileId),
    #[error("browser profile already has an active lease: {lease_id}")]
    ...
    #[error("browser lease {lease_id} is bound to another browser session")]
    LeaseSessionMismatch { lease_id: BrowserLeaseId },
    ...
    #[error("browser frame sequence changed: expected={expected}, provided={provided}")]
    FrameSequenceMismatch { expected: u64, provided: u64 },
