...
use super::types::{
    LocalProcessExecutionResponse, LocalProcessExecutorAffinity, LocalProcessExecutorCapability,
    LocalProcessExecutorDescriptor, LocalProcessExecutorHealth,
    ...
    LocalProcessVerifyResponse, LOCAL_PROCESS_PROTOCOL_VERSION,
...
use crate::{
    derive_final_report, WorkerExecutionFinalReport, WorkerExecutionIntent,
    WorkerExecutionIntentStep, WorkerExecutionTrace, WorkerStage, WorkerToolInvocation,
};
...
pub(super) fn loopback_capability() -> LocalProcessExecutorCapability {
    let session_id = std::env::var("MAGI_LOCAL_WORKER_SESSION_ID")
        .ok()
        .and_then(|value| if value.trim().is_empty() { None } else { Some(SessionId::new(value)) });
    let workspace_id = std::env::var("MAGI_LOCAL_WORKER_WORKSPACE_ID")
        .ok()
        .and_then(|value| if value.trim().is_empty() { None } else { Some(WorkspaceId::new(value)) });
    let process_model = {
        ...
        match process_model.as_str() {
            "shadow-loopback" | "shadow" => {
                LocalProcessExecutorProcessModel::ShadowLoopback
            }
    ...
    LocalProcessExecutorCapability {
        ...
        stage_matrix: LocalProcessExecutorStageMatrix {
            execute: LocalProcessWorkerExecutor::env_bool(
                "MAGI_LOCAL_WORKER_STAGE_EXECUTE",
                true,
            ),
            review: LocalProcessWorkerExecutor::env_bool(
                "MAGI_LOCAL_WORKER_STAGE_REVIEW",
                true,
            ),
            verify: LocalProcessWorkerExecutor::env_bool(
                "MAGI_LOCAL_WORKER_STAGE_VERIFY",
                true,
            ),
            repair: LocalProcessWorkerExecutor::env_bool(
                "MAGI_LOCAL_WORKER_STAGE_REPAIR",
                true,
            ),
        },
    ...
    let response = match request.kind {
        LocalProcessProtocolRequestKind::Probe(probe_request) => {
            ...
            if let Some(executor_request) = probe_request.executor_request.as_ref() {
                if let Err(error) =
                    LocalProcessWorkerExecutor::validate_executor_request(
                        &capability,
                        executor_request,
                    )
                {
                ...
                LocalProcessProtocolResponse {
                    ...
                    kind: LocalProcessProtocolResponseKind::Error(
                        WorkerExecutorFailure::remote_business(
                            "execution intent missing steps",
                        ),
                    ),
                }
                ...
                LocalProcessProtocolResponse {
                    ...
                    kind: LocalProcessProtocolResponseKind::Execute(
                        LocalProcessExecutionResponse {
                            trace: execute_intent_with_shadow_drivers(&execute.intent),
                        },
                    ),
                }
            }
        ...
        LocalProcessProtocolRequestKind::Review(review) => {
                ...
                )
            {
                ...
                let review_summary = trace
                    .final_report
                    .summary
                    .clone();
                LocalProcessProtocolResponse {
                    request_id: request.request_id,
                    kind: LocalProcessProtocolResponseKind::Review(
                        LocalProcessReviewResponse {
                            trace,
                            review_summary,
                        },
                    ),
        ...
        LocalProcessProtocolRequestKind::Verify(verify) => {
                ...
                )
            {
                ...
                LocalProcessProtocolResponse {
                    request_id: request.request_id,
                    kind: LocalProcessProtocolResponseKind::Verify(
                        LocalProcessVerifyResponse {
                            trace,
                            verification_status,
                            verify_summary,
                        },
        ...
        LocalProcessProtocolRequestKind::Repair(repair) => {
                ...
                )
            {
                ...
                LocalProcessProtocolResponse {
                    request_id: request.request_id,
                    kind: LocalProcessProtocolResponseKind::Repair(
                        LocalProcessRepairResponse {
                            trace,
                            repair_summary,
                        },
                    ),
