...
const BROWSER_RUNTIME_READY_EVENT = "magi-desktop:browser-runtime-ready";
...
contextBridge.exposeInMainWorld("magiDesktop", {
  runtime: "electron",
  surface: desktopSurface === "app" ? desktopSurface : null,
  windowId: desktopWindowId,
  getSnapshot: () => ipcRenderer.invoke("magi-desktop:get-snapshot"),
  setContext: (context: unknown) =>
    ipcRenderer.invoke("magi-desktop:set-context", context),
  submitLayoutIntent: (intent: unknown) =>
    ipcRenderer.invoke("magi-desktop:layout-intent", intent),
  activateBrowser: (request: unknown) =>
  ...
  waitForBrowserSurface: (request: unknown) =>
    ipcRenderer.invoke("magi-desktop:wait-for-browser-surface", request),
  activatePanel: (request: unknown) =>
    ipcRenderer.invoke("magi-desktop:activate-panel", request),
  setBrowserViewport: (request: unknown) =>
  ...
  openExternal: (url: string) =>
    ipcRenderer.invoke("magi-desktop:open-external", url),
  showContextMenu: (request: unknown) =>
    ipcRenderer.invoke("magi-desktop:show-context-menu", request),
  openWorkspaceFolder: (workspaceRootPathRef: string) =>
    ipcRenderer.invoke(
      "magi-desktop:open-workspace-folder",
      workspaceRootPathRef,
    ),
  revealWorkspaceFile: (request: unknown) =>
    ipcRenderer.invoke("magi-desktop:reveal-workspace-file", request),
  setAppearance: (appearance: unknown) =>
    ipcRenderer.invoke("magi-desktop:set-appearance", appearance),
  getAppVersion: () => ipcRenderer.invoke("magi-desktop:get-app-version"),
  getBrowserComponentInfo: () =>
    ipcRenderer.invoke("magi-desktop:get-browser-component-info"),
  ...
  onSnapshot: (listener: (snapshot: unknown) => void) =>
    subscribe(SNAPSHOT_EVENT, listener),
  onBrowserRuntimeReady: (listener: (event: unknown) => void) =>
    subscribe(BROWSER_RUNTIME_READY_EVENT, listener),
  onContext: (listener: (context: unknown) => void) =>
    subscribe(CONTEXT_EVENT, listener),
  onBrowserEvent: (listener: (event: unknown) => void) =>
    subscribe(BROWSER_EVENT, listener),
  onBrowserComponent: (listener: (snapshot: unknown) => void) =>
    subscribe(BROWSER_COMPONENT_EVENT, listener),
  onUpdate: (listener: (snapshot: unknown) => void) =>
    subscribe(UPDATE_EVENT, listener),
