...
const task_dependency_graph_1 = require("./task-dependency-graph");
...
class WorkerPool extends events_1.EventEmitter {
    ...
    queueCounter = 0;
    fileLockManager = new file_lock_manager_1.FileLockManager();
    cancelGeneration = 0;
    // 🆕 执行统计和降级相关
    executionStats;
    enableFallback;
     ...
     * 分发任务给指定 Worker
     */
    async dispatchTask(type, taskId, subTask, context, options) {
    ...
    async enqueueTask(type, taskId, subTask, context, options) {
     ...
     */
    async dispatchTaskWithRetry(type, taskId, subTask, context, options) {
        const state = {
     ...
     * 带超时的任务执行
     */
    async executeWithTimeout(type, taskId, subTask, context, options) {
        return new Promise((resolve, reject) => {
            const controller = new AbortController();
            const abortSignal = options?.abortSignal;
            const cleanup = (timeoutId, onAbort) => {
     ...
     * 🆕 记录执行统计
     */
    recordExecution(cli, taskId, subTaskId, success, duration, error) {
     ...
     */
    tryFallback(failedCli, triedClis) {
        ...
        // 没有统计数据时，使用简单的轮换策略
        const allClis = ['claude', 'codex', 'gemini'];
    ...
    async executeWithDependencyGraph(taskId, subTasks, context) {
        // 构建依赖图
        const graph = new task_dependency_graph_1.TaskDependencyGraph();
        // 添加所有任务到图中
        ...
        for (const batch of analysis.executionBatches) {
            console.log(`[WorkerPool] 执行批次 ${batch.batchIndex + 1}/${analysis.executionBatches.length}:`, batch.taskIds.join(', '));
            // 获取批次中的任务
            const batchTasks = batch.taskIds
                .map(id => graph.getTask(id)?.data)
            ...
            for (const result of batchResults) {
                graph.updateTaskStatus(result.subTaskId, result.success ? 'completed' : 'failed');
            }
            // 如果有任务失败，检查是否影响后续任务
     ...
     * 🆕 并行执行一批任务
     */
    async executeBatchParallel(taskId, subTasks, context, dependentsCount) {
        // 限制并行数量
        ...
        for (let i = 0; i < subTasks.length; i += maxParallel) {
            ...
            const chunkPromises = chunk.map(subTask => {
                ...
                return this.dispatchTaskWithRetry(subTask.assignedWorker || 'claude', taskId, subTask, context, { priority }).catch(error => ({
                    workerId: 'unknown',
