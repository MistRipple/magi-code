...
export class ContextCompressor {
  ...
  async compress(memory: MemoryDocument): Promise<boolean> {
    ...
    // 对长文本字段进行截断，快速减少大小
    const truncatedContent = this.applyPreventiveTruncation(content);
   ...
   * 对长文本字段进行截断，不使用 LLM
   */
  private applyPreventiveTruncation(content: MemoryContent): MemoryContent {
  ...
  private mergeCodeChanges(changes: MemoryContent['codeChanges']): MemoryContent['codeChanges'] {
    ...
    changes.forEach(change => {
      ...
      if (existing) {
        // 合并摘要
        existing.summary = `${existing.summary}; ${change.summary}`;
