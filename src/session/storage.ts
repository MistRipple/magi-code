...
export class SessionStorage {
  ...
  async load(id: string): Promise<Session | undefined> {
    ...
    return this.context.globalState.get<Session>(key);
  }
  ...
  async list(): Promise<SessionMeta[]> {
    return this.context.globalState.get<SessionMeta[]>(SESSION_LIST_KEY) || [];
  }

  /**
   * 更新会话列表
   */
  private async updateSessionList(session: Session): Promise<void> {
    const list = await this.list();
