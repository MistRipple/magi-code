...
本次实现只把 Git 管理能力暴露给普通主会话和 coordinator 主线；sidechain 子代理看不到
`git_*` 工具，执行层也会拒绝带 `worker_id` 的 Git mutation，避免子代理切换主 HEAD。
`magi-git` 已提供按指定 base commit 创建 detached/branch worktree 的结构化原语，但本次不把
worktree 自动分配逻辑耦合进 task dispatcher。需要并行写入隔离时，应由主对话先调用
`git_worktree_create` 建立明确路径和 branch，再把该执行目录交给 worker；不能让多个写代理
直接共享主 worktree。
...
`workspace_vcs` 提供与模型工具共用 `GitService` 的结构化 HTTP API，供后续交互界面接入。
本次交付重点是主模型可调用工具，不绑定具体分支面板实现；后端仍完整执行 dirty、drift、
运行 lease、强制删除和远程删除确认等安全检查。
