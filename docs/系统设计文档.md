...
> 版本: 0.5.1 | 最后更新: 2026-01-13

## 0. 设计重构声明（强制）

本次方案为“完整修复与必要重构”，不采取兼容性处理，不保留历史技术债。
所有旧流程与策略必须迁移至新策略内核统一驱动。
...
### 2.2.x 策略内核（Policy Engine）— 重构核心

Policy Engine 是编排系统唯一决策入口，统一负责：
- 工作流路径选择（轻量/标准/完整）
- 并行/串行决策与冲突域判断
- 验证强度选择（编译/诊断/测试等）
- 失败治理策略与恢复门槛
...
**轻量跨层合并原则**  
- 若修改跨前后端但属于“同一责任域的小改动”（例如：后端新增字段且前端只需展示该字段），默认合并为单个子任务，避免拆分成本高于收益。  
- 仅当跨层改动存在明确风险（接口不确定、业务逻辑复杂、影响范围大）才拆分为多个 Worker。  
...
| 冲突域类型 | 识别依据 | 典型范围 | 调度策略 |
| --- | --- | --- | --- |
| 模块域 | 同一业务模块 | `src/modules/user/**` | 默认串行 |
| 接口域 | 同一 API/契约 | `src/api/auth.ts` + `src/pages/login.tsx` | 先契约后实现 |
| 配置域 | 配置/依赖文件 | `package.json`、`tsconfig.json` | 强制串行 |
| 目录域 | 同一目录多文件 | `src/components/**` | 风险评估后并行 |

**冲突域标注规则**
- 子任务必须填写 `conflictDomain` 与 `dependencyChain`
- 若无法确定冲突域，默认视为高风险，进入标准或完整路径
...
为避免“重复输出、错位输出、丢失输出”，消息与会话必须遵循统一约束：

**统一消息入口**
- UI 仅接收两类消息：`streamEvent`（流式增量）与 `orchestratorMessage`（阶段结论/错误/提示）。
- `stateUpdate` 仅用于统计与列表（任务/变更/会话）更新，不触发主对话或 CLI 对话内容渲染。

**事件结构约束**
- 所有消息必须携带 `sessionId`，UI 只渲染当前会话的消息。
- `streamEvent.target` 必须明确：`thread`（主对话）或 `cli`（子代理面板）。
- `streamEvent.source` 必须明确：`orchestrator` 或 `worker`。

**角色隔离**
- 主对话（Thread）只允许 `orchestrator` 的内容进入。
- Worker 输出只能进入对应 CLI 面板，禁止回流主对话。

**会话一致性**
- 会话模型单一化：对话、任务、快照统一绑定同一个 Session。
- 切换会话必须静默中断当前任务，避免旧会话提示污染新会话。

**渲染节奏**
- 流式内容采用增量更新，不做全量 DOM 重建。
- 仅当“工具调用数量变化/思考块数量变化”时触发结构性重渲染。

---

### 2.8 风险策略内核（RiskPolicy）

**目标**：把“风险评分、路径选择、Hard Stop、验证梯度”收敛为一个统一策略内核，避免分散规则与重复判断。  

**核心能力**
- 风险评分（低/中/高）
- 路径选择（轻量/标准/完整）
- Hard Stop 触发条件（仅在中高风险）
- 验证梯度（高风险才执行完整验证）

**最小策略集合**
- Low：单 Worker，轻量路径，无 Hard Stop，基础验证（可选）
- Medium：必要时拆分，Hard Stop 可触发，常规验证
- High：强制拆分/联调/互检，Hard Stop 必须，完整验证
...
采用 **画像驱动** 的设计思路，不固定 CLI 角色，而是：
- 画像系统内置任务分类与默认 Worker
- 用户可通过画像配置面板调整分类默认 Worker
- 系统根据画像配置与 CLI 可用性动态选择
...
用户通过画像配置面板调整分类默认 Worker，配置写入 `~/.multicli/`（claude.json/codex.json/gemini.json/categories.json）。
...
### 6.x 风险阈值驱动 Hard Stop（重构）

Hard Stop 不再固定触发，由风险阈值决定：
- 低风险任务自动执行
- 中风险任务可选确认
- 高风险任务强制确认

该决策由 Policy Engine 统一输出。
...
系统必须通过风险评分选择路径，禁止人工或临时条件分支。

**路径定义**
- 轻量路径：P1 → P3 → P7（单文件、低风险、单 CLI 可闭环）
- 标准路径：P1 → P2 → P3 → P5 → P7（常规任务）
- 完整路径：P1 → P7 全流程（跨域/高风险/多依赖）
...
所有任务必须命中一个路径，不允许“混合路径”。

**风险评分规则（建议）**

| 维度 | 评分规则 | 权重 |
| --- | --- | --- |
| 文件数 | 1-2=1分，3-5=2分，6+=3分 | 2 |
| 跨模块数量 | 0=0分，1=1分，2+=2分 | 3 |
| 接口变更 | 无=0分，有=3分 | 4 |
| 配置/依赖修改 | 无=0分，有=3分 | 4 |
| 历史失败率 | <10%=0分，10-30%=1分，>30%=2分 | 2 |
...
| 路径类型 | 判断条件 | 执行方式 |
| -------- | -------- | -------- |
| **轻量路径** | 单文件、低风险、单 CLI 可闭环 | P1 → P3 → P7 |
| **标准路径** | 常规开发任务 | P1 → P2 → P3 → P5 → P7 |
| **完整路径** | 多领域、高风险、多依赖 | P1 → P7 全流程 |

**示例**：
- "修复 parseConfig 函数的空指针问题" → 轻量路径
- "优化登录页面的样式" → 轻量路径
- "添加用户登录功能，包括后端 API 和前端页面" → 完整路径
- "重构用户模块，拆分为认证和资料两个服务" → 完整路径
...
验证等级必须由 Policy Engine 决策，禁止手工插入流程分支。
...
- **计划确认（Hard Stop）**：只在主对话内显示确认卡片，不使用模态弹窗。
