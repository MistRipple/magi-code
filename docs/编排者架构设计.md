...
> 目标：编排者负责“流程与决策”，Worker 负责“实现与验证”，任何越界都视为流程错误。

#### 角色职责边界

- **Orchestrator（编排者）**
  - 负责：需求澄清、任务拆分、依赖编排、阶段推进、验收标准、最终总结
  - 不负责：具体业务代码实现、直接修改文件、替 Worker 下结论
- **Worker Claude（架构/集成/联调）**
  - 负责：系统架构与目录结构、接口契约、联调方案、跨端冲突识别
  - 不负责：替编排者调度任务
- **Worker Codex（后端）**
  - 负责：后端接口实现、数据层、鉴权、错误处理、测试
  - 不负责：前端 UI 或联调决策
- **Worker Gemini（前端）**
...
1. **编排权只属于 Orchestrator**：Worker 不得直接对其他 Worker 下达任务。
2. **架构与联调由 Claude Worker 承担**：避免前后端“各写各的”。
3. **依赖关系必须显式化**：后端/前端任务依赖架构任务完成。
4. **自检与互检只做质量验证，不做调度**：调度仍由 Orchestrator 执行。
