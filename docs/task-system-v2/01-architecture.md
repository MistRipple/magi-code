...
- 中等任务：明确目标下的实现、修复、调研、验证，可能需要拆分和多代理协作。
...
- 中等任务必须有可观察的执行链、可并行推进的代理任务和可回收的结构化代理结果。
  ...
  └─ classify route = task
       └─ create ExecutionChain
            └─ create root Task(LocalAgent)
                 └─ TaskRunner dispatch
                      ├─ single-agent task：root 代理直接完成
                      └─ coordinated task：主代理通过 agent_spawn 生成 child tasks
                           └─ parent 通过 agent_wait 收集 child terminal outcome
```
...
| 形态 | 触发条件 | 推进方式 |
|------|----------|----------|
| Single-agent task | 目标明确、单代理可闭环 | root task 直接执行、验证、完成 |
...
- `agent_spawn` 只创建代理并投递初始任务消息；parent 需要结果时通过 `agent_wait` 收集 child task 终态。
- root task 完成必须有输出摘要和必要验证证据。

中等任务的成功标准是：用户能看到当前任务进度、代理分工、阻塞点、最终验证结果。
...
运行时输入通道。用户继续输入、父任务消息、系统 followup 都进入 mailbox，在下一次 Turn 边界注入模型上下文。代理 spawn 的终态结果不走 mailbox 旁路；父任务需要结果时通过 `agent_wait` 从 TaskStore 收集。
       ...
       └─ classify: task
            └─ dispatch_submission
                 ...
                 └─ TaskRunner.run_cycle(root)
                      ...
                      ├─ agent_wait collects child terminal outcome
...
3. MissionCharter 使用 `draft/frozen` 生命周期。
4. Checkpoint 作为可恢复快照协议，不只是摘要日志。
5. Long Mission 使用显式 `TaskTier::LongMission`，`background_allowed` 只作为执行策略字段。
6. 代理结果由 `agent_wait` 读取 TaskStore 终态结果，前端仅把 `agent_spawn` 呈现为主线工具卡片。
