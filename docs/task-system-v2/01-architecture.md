...
| `ExecutionChain` | 当前可恢复执行链，连接 UI、root task、worker lane、恢复信息 | 不作为业务工作单元 |
...
| `Thread` | 单个 worker/task 的持久化消息历史 | 不混用不同 task 历史 |
       ...
       └─ create ExecutionChain
            └─ create root Task(LocalAgent)
                 └─ TaskRunner dispatch
                      ...
                      └─ coordinated task：coordinator 通过 agent_spawn 生成 child tasks
                           └─ child result -> parent mailbox -> parent next turn
```
...
| Coordinated task | 需要拆解、并行、子结果回收 | root task 使用 coordinator role，spawn 子 task 并汇总 |
...
- child task 完成后必须投递 `AgentResult` 到 parent task 的 mailbox。
...
运行时输入通道。用户继续输入、父任务消息、子任务结果、系统 followup 都进入 mailbox，在下一次 Turn 边界注入模型上下文。
...
统一发布模型 token、工具事件、worker 状态和任务完成事件。前端 projection 从 stream 派生，不直接拼装运行时私有状态。
...
Coordinator 是 prompt-as-code。模型可以通过 `agent_spawn`、`send_message`、`task_stop` 协调工作，但 runtime 必须限制工具可见性、spawn 深度、人审状态和权限边界。
...
- **Thread 不串线**：每个 worker/task 使用独立 thread；历史 worker thread 只做审计，不注入新 task 上下文。
