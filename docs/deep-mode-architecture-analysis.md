...
| worktree 运行保障 | 已落地（本轮） | 持续优化 | 已补 merge 冲突结构化解释、孤儿 worktree/分支对账清理、冲突后修复微任务入口；非 Git 工作区写任务自动降级为主工作区单写串行模式 | `src/workspace/worktree-manager.ts`、`src/orchestrator/core/worker-pipeline.ts`、`src/orchestrator/core/dispatch-manager.ts` |
...
| 7.6-worktree / 非 Git 写隔离分级 | 已完成（本轮） | Git 工作区继续使用 worktree 物理隔离；非 Git 工作区写任务自动降级为主工作区单写串行模式，不再把仓库前置条件暴露成用户错误 | `npm run -s verify:e2e:worktree-runtime-guardrails`、`npm run -s verify:e2e:non-git-write-serialization` |
...
| 7.6-deep 自动续跑门禁收紧 | 已完成（本轮） | 自动续跑仅对“未完成 required todo 或结构化 `runtime.nextSteps` 任务步骤”生效；已移除正文“下一步建议”文本解析，非任务对话/能力描述不再触发续跑；当当前 phase 已完成但 mission 仍存在结构化后续阶段时，允许 deep 自动续跑继续推进，而不是错误停在阶段边界；用户侧输出改为简洁摘要，降低信息噪声 | `npm run -s verify:e2e:auto-deep-followup` |
